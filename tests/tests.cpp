...
//Some of the transparently serializable tests check things on common platforms. These tests require an 8 bit byte
//due to hardcoded constants.
static_assert(common_platform::transparently_serializable<u8> and std::numeric_limits<u8>::digits == 8);
...
static constexpr auto field9 = [x = field3.get_value<1>()] { std::remove_const_t<decltype(field3)> f9{}; f9.set_value<1>(x); f9.s[8] |= std::byte{ 0x08 };  return f9; }();
...
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<i32>);
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<const i32>);
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<volatile i32>);
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<const volatile i32>);
using B1 = B<I64<32>, U8<5>, I16<7>>;
static_assert(common_platform::is_transparently_serializable_v<B1>);
struct T1 { B1 a; u16 b; };
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<T1>);
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<const T1>);
...
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<T4>);
struct T5 { alignas(8) B<I64<64>> a; u16 b; u8 c, d; u32 e; };
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<T5>);
struct T6 { alignas(4) struct { u16 a[2]; } a; u32 b; };
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<T6>);
struct T7 { struct { alignas(4) u16 a[2]; } a; u32 b; };
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<T7>);
...
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<T5, T6>);
static_assert(not common_platform::is_common_platform or common_platform::is_transparently_serializable_v<T5, T6[]>);
...
auto test() noexcept
{
	auto fun = [](const char*, std::uint32_t u, common_platform::bitfield<common_platform::float_constant<float, 24, 8, u32>> p[], std::size_t n)
	{
		std::cout << u << std::endl;
...
int main()
{
	...
	test<common_platform::bitfield<U32<32>>, common_platform::bitfield<common_platform::float_constant<float, 24, 8, u32>>[]>();

	struct foobar { common_platform::bitfield<U32<32>> a; common_platform::bitfield<common_platform::float_constant<float, 24, 8, u32>> b[2]; };
	alignas(foobar) std::byte buf[sizeof(std::uint32_t) + sizeof(float[2])]{};

	auto fun = [](std::uint32_t uu, common_platform::bitfield<common_platform::float_constant<float, 24, 8, u32>>(&a)[2])
	{
	...
	static_assert(data_serialization::flex_element_size_v<foobar, decltype(fun)> == 0);
	assert(73 == data_serialization::apply<foobar>(fun, buf, sizeof(buf)));

	auto fun2 = [](std::uint32_t uu, common_platform::bitfield<common_platform::float_constant<float, 24, 8, u32>> p[], std::size_t n) {
		std::cout << uu << std::endl;
	...
	static_assert(data_serialization::flex_element_size_v<foobar, decltype(fun2)> == 4);
